...
public class TestForManyByHessianMain extends BaseTest {

	private static final Log logger = LogFactory
			.getLog(TestForManyByHessianMain.class);
	...
	public void testQueryInfoPool() {
		try {
			...
			for (int i = 0; i < threadNum; i++) {
				executor.execute(new TestClientByHessianThreadRun(
						new Integer(i), main, runNum));
			}
			try {
				main.await();
