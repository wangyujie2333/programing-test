...
import yspay.cust.bean.entity.MerchantInfo;
import yspay.cust.remoteserver.iface.IMerchantInfo;
...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			// hessian rpc测试
			try {
				IMerchantInfo merchantInfo = (IMerchantInfo) applicationContext
						.getBean("merchantInfo");

				MerchantInfo merchant = merchantInfo
						.getMerchantInfoById("2013042300019263");

				if (merchant != null) {
					System.out.println("商户号：" + merchant.getPartnerCode());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
